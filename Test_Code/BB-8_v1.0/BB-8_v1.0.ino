...
#define AUX 13        // Auxiliary channel
...
#define cmdA 'H'      // UART-command for auxiliary channel (Perhaps BB-8's mast?)
...
char A_Data[1];       // array data for auxiliary channel
...
void setup() {
  ...
  pinMode(AUX, OUTPUT);    // additional channel
...
void timer_init() {
  uint8_t sw_autoOFF = EEPROM.read(0);   // read EEPROM "is activated or not stopping when losing connection"
...
void loop() {
  if (Serial.available() > 0) {          // if received UART data
    ...
    else if(incomingByte == cmdA) {      // if received data for additional channel
      command = cmdA;
    ...
    if(command == cmdL && incomingByte != cmdL){
      ...
      Control4WD(atoi(L_Data),atoi(R_Data),atoi(A_Data));
      delay(10);
    }
...
void Control4WD(int mLeft, int mRight, uint8_t Aux){
  ...
  digitalWrite(AUX, Aux);           // additional channel
}
